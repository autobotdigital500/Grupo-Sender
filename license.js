...
(function () {
    ...
    async function checkLicenseOnline(key) {
        ...
        try {
            // Evita cache do navegador adicionando timestamp
            const url = `${SUPABASE_URL}/rest/v1/licenses?license_key=eq.${encodeURIComponent(key)}&select=*,extensions:extension_id(name)&_t=${Date.now()}`;
    ...
    // ─── REVALIDAÇÃO PERIÓDICA (A CADA 2 MINUTOS) ─────────────────────────────────
    setInterval(async () => {
        if (document.body.getAttribute('data-license-authorized') === 'true') {
            const cachedKey = localStorage.getItem(LS_KEY);
            if (cachedKey) {
                const r = await checkLicenseOnline(cachedKey);
                if (!r.valid) {
                    localStorage.removeItem(LS_KEY);
                    localStorage.removeItem(LS_CACHE);
                    document.body.removeAttribute('data-license-authorized');
