...
(function (root) {
    ...
    const REQUIRED_EXTENSION_NAME = 'sender';
    ...
    const BLOCKER_HTML = `
        <div id="${BLOCKER_ID}" style="position:fixed!important;top:0!important;left:0!important;width:100vw!important;height:100vh!important;background:#0f172a!important;z-index:2147483647!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;font-family:sans-serif!important;color:white!important;">
            <div style="background:#1e293b;padding:2rem;border-radius:12px;border:1px solid #334155;max-width:400px;text-align:center;box-shadow:0 20px 25px -5px rgba(0,0,0,0.5);">
                <div style="margin-bottom:1.5rem;"><img src="logo.png" style="width:64px;height:64px;" onerror="this.style.display='none'"></div>
                ...
                <input type="text" id="ext-license-input" placeholder="Chave de Licença..." style="width:100%;padding:12px;margin-bottom:15px;border-radius:6px;border:1px solid #475569;background:#0f172a;color:white;text-align:center;font-size:16px;outline:none;" autocomplete="off" />
    ...
    // Função que paralisa o motor se for bloqueado
    function killEngine() {
        try {
            if (window.chrome && chrome.runtime && chrome.runtime.sendMessage) {
                chrome.runtime.sendMessage({ action: 'taskStop' }).catch(() => {});
            }
            if (window.chrome && chrome.storage && chrome.storage.local) {
                chrome.storage.local.remove(['license_key', 'license_cache']);
            }
            if (typeof io !== 'undefined' && window.socket) {
                window.socket.emit('chrome_message', { action: 'taskStop' });
            }
        } catch(e) {}
    }
    ...
    async function handleLicenseValidation() {
        ...
        if (result.valid) {
            ...
            if (window.chrome && chrome.runtime && chrome.runtime.sendMessage) {
                chrome.runtime.sendMessage({ action: 'LICENSE_VALIDATED', sig: _GCRM_SIG }).catch(()=>{});
            }
        } else {
    ...
    if (typeof window !== 'undefined') {
        ...
        const initProcess = () => {
            if (!window.chrome || !chrome.storage) return injectUI();
            ...
            chrome.storage.local.get(['license_key', 'license_cache'], async (res) => {
                ...
                if (isCacheValid) {
                    ...
                    // Valida no fundo para pegar suspensões em background
                    const result = await checkLicenseOnline(licenseKey);
                    if (!result.valid) {
                        killEngine();
                        document.body.setAttribute('data-license-authorized', 'false');
        ...
        // Revalidação a cada 2 minutos em background (para travar painel aberto)
        setInterval(() => {
            if (document.body.getAttribute('data-license-authorized') === 'true') {
                if (window.chrome && chrome.storage && chrome.storage.local) {
                    chrome.storage.local.get(['license_key'], async (res) => {
                        if (res.license_key) {
                            const result = await checkLicenseOnline(res.license_key);
                            if (!result.valid) {
                                killEngine();
                                document.body.setAttribute('data-license-authorized', 'false');
                                injectUI();
